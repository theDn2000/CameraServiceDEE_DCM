...
class ColorDetector:
    ...
    def MarkFrameForCalibration (self, frame):
        ...
        cv.putText(img=frame, text=str(self.yellowtmp), org=(x1-50, y1), fontFace=cv.FONT_HERSHEY_TRIPLEX, fontScale=2,
                   color=(255,255,255), thickness=1)

        cv.circle(frame, (x2, y1), 100, (0, 255, 0), 3)
        cv.putText(img=frame, text=str(self.greentmp), org=(x2-50, y1), fontFace=cv.FONT_HERSHEY_TRIPLEX, fontScale=2,
                   color=(255,255,255), thickness=1)

        cv.circle(frame, (x3, y1), 100, (240, 106, 23), 3)
        cv.putText(img=frame, text=str(self.blueStmp), org=(x3-50, y1), fontFace=cv.FONT_HERSHEY_TRIPLEX, fontScale=2,
                   color=(255,255,255), thickness=1)

        cv.circle(frame, (x1, y2), 100, (250, 240, 30), 3)
        cv.putText(img=frame, text=str(self.blueLtmp), org=(x1-50, y2), fontFace=cv.FONT_HERSHEY_TRIPLEX, fontScale=2,
                   color=(255,255,255), thickness=1)

        cv.circle(frame, (x2, y2), 100, (139, 1, 240), 3)
        cv.putText(img=frame, text=str(self.pinktmp), org=(x2-50, y2), fontFace=cv.FONT_HERSHEY_TRIPLEX, fontScale=2,
                   color=(255,255,255), thickness=1)

        cv.circle(frame, (x3, y2), 100, (240, 29, 140), 3)
        cv.putText(img=frame, text=str(self.purpletmp), org=(x3-50, y2), fontFace=cv.FONT_HERSHEY_TRIPLEX, fontScale=2,
                   color=(255,255,255), thickness=1)
        return frame
